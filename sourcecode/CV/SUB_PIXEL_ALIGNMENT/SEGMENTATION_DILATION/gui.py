    ...
    def load_images(self):
        ...
        if file_paths:
            for file_path in file_paths:
                ...
                rois, resized_rois = ImageSegmenter(new_image, self.colours).run()

                for roi in rois:
                    self.images.append(roi)

                for resized_roi in resized_rois:
                    self.segmented_images.append(resized_roi)
    ...
    def next_image(self):
        if len(self.images) > 1:
            if len(self.images) > self.current_image:
                ...
                segmented_image = self.segmented_images[self.current_image - 1]
