...
class ImageSegmenter:
    ...
    def label_dots(self, cropped_dots, ordered_HexaTargets):
        ...
        for (cropped_dot, dot_info) in cropped_dots_with_info:
            ...
            print(f"dot_info before correction: {dot_info}")

            dot_info[0] = dot_info[0] * scale_factor
            dot_info[1] = dot_info[1] * scale_factor
            dot_info[2] = dot_info[2] * scale_factor
            dot_info[3] = dot_info[3] * scale_factor
            dot_info[4] = dot_info[4] * scale_factor
            dot_info[5] = dot_info[5] * scale_factor
            dot_info[6] = dot_info[2] * scale_factor * scale_factor

            print(f"dot_info after correction: {dot_info}")

        resized_dots_with_info = list(zip(resized_dots, dots_info))
    ...
    def run(self):
        ...
        labelled_dots = self.label_dots(cropped_dots, ordered_HexaTargets)


        ### ORIGINAL

        colour_segmented_dots = []
        ...
        for (dot, info) in cropped_dots_with_info:
            ...
            if colour == "red":
                ...
                hsv_dot[red_segments == 255] = [0,255,255]  # Bright red
                bgr_dot = cv2.cvtColor(hsv_dot, cv2.COLOR_HSV2BGR)
                colour_segmented_dots.append(bgr_dot)
            ...
            if colour == "green":
                ...
                hsv_dot = cv2.cvtColor(dot.copy(), cv2.COLOR_BGR2HSV)
                # Setting bright colours for the segmented regions
                hsv_dot[green_segments == 255] = [60,255,255]  # Bright green
                bgr_dot = cv2.cvtColor(hsv_dot, cv2.COLOR_HSV2BGR)
                colour_segmented_dots.append(bgr_dot)
            ...
            if colour == "blue":
                ...
                hsv_dot = cv2.cvtColor(dot.copy(), cv2.COLOR_BGR2HSV)
                # Setting bright colours for the segmented regions
                hsv_dot[blue_segments == 255] = [120,255,255]  # Bright blue
                bgr_dot = cv2.cvtColor(hsv_dot, cv2.COLOR_HSV2BGR)
                colour_segmented_dots.append(bgr_dot)

        resized_dots = []
        scale_factor = 16
        for dot in colour_segmented_dots:
            # Scale up the cropped dot image by 16 times
            resized_dot = cv2.resize(dot.copy(), None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_NEAREST)
            resized_dots.append(resized_dot)
        ...
        for (dot, info) in cropped_dots_with_info:
            ...
            if colour == "red":
                ...
                hsv_dot = cv2.cvtColor(dot.copy(), cv2.COLOR_BGR2HSV)
                # Setting bright colours for the segmented regions
                hsv_dot[dilated_red_segments == 255] = [0,255,255]  # Bright red
                bgr_dot = cv2.cvtColor(hsv_dot, cv2.COLOR_HSV2BGR)
                dilated_colour_segmented_dots.append(bgr_dot)
            ...
            if colour == "green":
                ...
                hsv_dot = cv2.cvtColor(dot.copy(), cv2.COLOR_BGR2HSV)
                # Setting bright colours for the segmented regions
                hsv_dot[dilated_green_segments == 255] = [60,255,255]  # Bright green
                bgr_dot = cv2.cvtColor(hsv_dot, cv2.COLOR_HSV2BGR)
                dilated_colour_segmented_dots.append(bgr_dot)
            ...
            if colour == "blue":
                ...
                hsv_dot = cv2.cvtColor(dot.copy(), cv2.COLOR_BGR2HSV)
                # Setting bright colours for the segmented regions
                hsv_dot[dilated_blue_segments == 255] = [120,255,255]  # Bright blue
                bgr_dot = cv2.cvtColor(hsv_dot, cv2.COLOR_HSV2BGR)
                dilated_colour_segmented_dots.append(bgr_dot)

        dilated_resized_dots = []
        scale_factor = 16
        for dot in dilated_colour_segmented_dots:
            # Scale up the cropped dot image by 16 times
            resized_dot = cv2.resize(dot.copy(), None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_NEAREST)
            dilated_resized_dots.append(resized_dot)
        ...
        return resized_dots, dilated_resized_dots